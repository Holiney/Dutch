...
export default function StudyWordsDeck({ words, onBack }: StudyWordsDeckProps) {
  ...
  const toggleKnown = () => {
    ...
    setKnownWords((prev) =>
      prev.includes(currentWord.dutch)
        ? prev.filter((word) => word !== currentWord.dutch)
        : [...prev, currentWord.dutch],
    );
  };
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="rounded-3xl border border-indigo-100 bg-white p-8 text-center shadow-xl">
        ...
        <p className="mt-2 text-sm text-stone-500">{currentWord.transcription}</p>
        <p className="mt-6 text-2xl font-medium text-indigo-600">{currentWord.translation}</p>
      </div>

      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <button
          type="button"
          onClick={() => setIndex((prev) => (prev - 1 + words.length) % words.length)}
          className="inline-flex items-center justify-center gap-2 rounded-xl border border-stone-200 bg-white px-4 py-3 text-sm font-medium text-stone-700 hover:bg-stone-50"
        >
